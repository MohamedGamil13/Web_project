...
const HOTELS = [
  {
    ...
    starRating: 4,
    description: 'A small boutique hotel near the river with a rooftop terrace and city views.',
  ...
  },
  {
    ...
    starRating: 5,
    description: 'Five-star riverside hotel with a Michelin-starred restaurant and full spa.',
  ...
  },
  {
    ...
    starRating: 4,
    description: 'Stone-built lodge a five minute walk from the castle. Includes a whisky bar.',
...
async function seed() {
  ...
  await Promise.all([User.deleteMany({}), Hotel.deleteMany({}), Room.deleteMany({})]);
  ...
  console.log(`[seed] users: ${admin.email}, ${alice.email}, ${bob.email} (password: Password1)`);
  ...
  for (const data of HOTELS) {
    ...
    console.log(`[seed] hotel: ${hotel.name} (${hotel.city}) — ${data.rooms.length} room types`);
  }
