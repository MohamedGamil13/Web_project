...
async function shapeReservation(reservationDoc) {
  ...
  return {
    ...
    hotel: hotel
      ? { id: hotel._id.toString(), name: hotel.name, city: hotel.city, country: hotel.country }
      : { id: r.hotelId },
    room: room
      ? {
...
export async function listMyReservations(userId) {
  const reservations = await Reservation.find({ userId }).sort({ checkIn: -1 }).lean();
...
export async function cancelReservation(userId, id) {
  ...
  if (reservation.checkIn <= new Date()) {
    throw ApiError.conflict('Cannot cancel a reservation that has already started');
  }
