...
export const getOne = asyncHandler(async (req, res) => {
  const reservation = await reservationsService.getReservation(req.user.id, req.params.id);
  ok(res, reservation);
});

export const cancel = asyncHandler(async (req, res) => {
  const reservation = await reservationsService.cancelReservation(req.user.id, req.params.id);
  ok(res, reservation);
});
