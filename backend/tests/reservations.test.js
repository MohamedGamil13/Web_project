...
describe('Reservation routes are auth-protected', () => {
  ...
  it('returns 401 on GET /reservations/:id with no token', async () => {
    const res = await request(app).get('/api/v1/reservations/507f1f77bcf86cd799439011');
    expect(res.status).toBe(401);
  });

  it('returns 401 on PATCH /reservations/:id/cancel with no token', async () => {
    const res = await request(app).patch('/api/v1/reservations/507f1f77bcf86cd799439011/cancel');
    expect(res.status).toBe(401);
  });
});
...
describe('createReservationSchema — body validation', () => {
  it('rejects missing fields', async () => {
    ...
    expect(fields).toEqual(expect.arrayContaining(['roomId', 'checkIn', 'checkOut', 'guests']));
  });
