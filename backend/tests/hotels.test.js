...
describe('GET /api/v1/hotels — query validation', () => {
  it('rejects maxPrice < minPrice with 422', async () => {
    const res = await request(app).get('/api/v1/hotels').query({ minPrice: 200, maxPrice: 100 });
    expect(res.status).toBe(422);
    expect(res.body.error.code).toBe('VALIDATION_ERROR');
  });

  it('rejects pageSize > 50 with 422', async () => {
    const res = await request(app).get('/api/v1/hotels').query({ pageSize: 100 });
    expect(res.status).toBe(422);
  });

  it('rejects an unknown sort key', async () => {
    const res = await request(app).get('/api/v1/hotels').query({ sort: 'random' });
    expect(res.status).toBe(422);
  });
});
...
describe('Hotel admin mutations require auth + admin role', () => {
  ...
  it('returns 403 with a non-admin user token on POST /hotels', async () => {
    const userToken = jwt.sign({ sub: '507f1f77bcf86cd799439011', role: 'user' }, env.jwtSecret);
