...
export function ReservationCard({ reservation, onCancel, cancellable }) {
  const { hotel, room, checkIn, checkOut, nights, guests, totalPrice, status } = reservation;
      ...
      <CardContent sx={{ flex: 1 }}>
        <Stack spacing={1.5}>
          <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={1}>
            ...
            {status === 'cancelled' && (
              <Chip label="Cancelled" size="small" variant="outlined" sx={{ flexShrink: 0 }} />
            )}
            {isPastStay && <Chip label="Past" size="small" sx={{ flexShrink: 0 }} />}
          </Stack>
          <Stack
            direction="row"
            spacing={0.5}
