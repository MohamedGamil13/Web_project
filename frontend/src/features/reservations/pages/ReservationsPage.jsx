...
function isUpcoming(reservation) {
    return reservation.status === 'active' && new Date(reservation.checkIn) > new Date();
}

function isRecentlyCancelled(reservation) {
    if (reservation.status !== 'cancelled') return false;
    if (!reservation.cancelledAt) return false;
    return Date.now() - new Date(reservation.cancelledAt).getTime() <= RECENT_CANCELLED_WINDOW_MS;
}
                    ...
                    <Card variant="outlined">
                        <CardContent>
                            ...
                            <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                                ...
                                <Button component={RouterLink} to="/reservations/history" variant="outlined">
                                    Past reservations
                                </Button>
                            </Stack>
                        </CardContent>
