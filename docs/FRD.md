...
- AC:
  ...
  - Email must be unique (case-insensitive); password min 8 chars, at least one letter and one number.
  - On success: account created, JWT returned, user redirected to home as logged in.
  - On duplicate email: HTTP 409 with field-level error shown next to the email input.

**A2. Login**
- *As a* user *I want* to log in *so that* I can access my account.
- AC:
  - Form fields: email, password.
  - On success: JWT stored, user redirected to the page they tried to access (or home).
  - On wrong credentials: HTTP 401, generic "Invalid email or password" message; no field-level leak.

**A3. Logout**
- *As a* user *I want* to log out *so that* my session is cleared.
- AC: token cleared client-side; protected routes redirect to /login.

**A4. Session persistence**
- AC: refreshing the page keeps the user logged in until token expiry (7d). Expired token → user is logged out and redirected to /login.

### Epic B — Profile
**B1. View profile**
- AC: `/profile` shows name, email, member-since date.

**B2. Edit profile**
- AC: name and email editable; email change re-checks uniqueness; success toast on save; validation errors shown inline.
